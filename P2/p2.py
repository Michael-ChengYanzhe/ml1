...
def main():
	...
	rawData = pd.read_csv('mobileprice.csv')
	# rawData.type = rawData.type.replace({'white': 1.0, 'red': 0.0})
	dropNaData = rawData.dropna(axis = 0)
	features = list(dropNaData)[:-1]
	target = dropNaData.iloc[:,-1]
	data = dropNaData.loc[:, dropNaData.columns != "price_range"]
...
def boosting(xTrain, xTest, yTrain, yTest):
	...
	param_test ={"n_estimators": [1, 10, 100, 200, 500, 700, 1000]}
	...
	plot_clf = AdaBoostClassifier(base_estimator = dtc, **gs_classifier.best_params_)
...
def knn(xTrain, xTest, yTrain, yTest):
	...
	plt = plot_learning_curve(plot_clf,"KNN", "knn_best_", xTrain, yTrain, train_sizes=np.linspace(0.05, 1.0, 50))
	...
	for k in neighbors_num:
		...
		plt = plot_learning_curve(plot_clf,"KNN_"+str(k), str(k)+"nn_", xTrain, yTrain, train_sizes=np.linspace(0.05, 1.0, 50)) 
...
def plot_learning_curve(clf, title, file_name, xTrain, yTrain,
	...
	plt.legend(loc="best")
	plt.savefig('ex2_' + file_name + 'curve.png')
	plt.tight_layout()
